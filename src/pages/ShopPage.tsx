    ...
    <div className="mx-auto max-w-1440 px-4 py-8 sm:px-6">
      ...
      {(q || category || gender || maxPrice) && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {q && (
            <button onClick={() => setParam("q", "")} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium hover:bg-neutral-200">
              "{q}" <X className="h-3 w-3" />
            </button>
          )}
          {categoryName && (
            <button onClick={() => setParam("category", "")} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium hover:bg-neutral-200">
              {categoryName} <X className="h-3 w-3" />
            </button>
          )}
          {gender && (
            <button onClick={() => setParam("gender", "")} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium hover:bg-neutral-200">
              {GENDER_LABELS[gender]} <X className="h-3 w-3" />
            </button>
          )}
          {maxPrice && (
            <button onClick={() => setMaxPrice("")} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs font-medium hover:bg-neutral-200">
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 max-w-[85vw] overflow-y-auto bg-white p-5 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button onClick={() => setFiltersOpen(false)} aria-label="Close filters" className="rounded-md p-2 hover:bg-neutral-100">
                <X className="h-5 w-5" />
              </button>
            </div>
            {filterPanel}
