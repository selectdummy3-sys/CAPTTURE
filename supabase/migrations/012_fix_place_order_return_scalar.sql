-- ============================================================
-- CAPPTURE — Fix place_order RETURN scalar subquery
-- ============================================================
-- Regression: `return (select * from public.orders where id = v_order_id)`
-- fails with "subquery must return only one column" because a parenthesised
-- subquery in a RETURN expression is treated as a scalar subquery. Selecting
-- the whole row as a single composite (`select o from ...`) fixes it.
