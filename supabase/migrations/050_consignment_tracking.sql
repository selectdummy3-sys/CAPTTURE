-- ============================================================
-- CAPPTURE — Consignment tracking
-- One checkout spanning multiple sellers creates multiple orders.
-- All of them share a single consignment_id and a single CAPPTURE-
-- generated tracking_number so the customer can track every parcel
-- under one code. Tracking is no longer seller-entered.
-- ============================================================

alter table public.orders
  add column if not exists consignment_id uuid;

create index if not exists orders_consignment_idx on public.orders (consignment_id);

-- ---------- place_order(): accept consignment + tracking ----------
-- Replace the canonical signature; old overload is dropped so PostgREST
-- resolves the RPC unambiguously (same approach as migration 040).
drop function if exists public.place_order(uuid, jsonb, text, jsonb, jsonb, text, text, text, uuid, text);
...
revoke all on function public.place_order(uuid, jsonb, text, jsonb, jsonb, text, text, text, uuid, text, uuid, text) from public;
grant execute on function public.place_order(uuid, jsonb, text, jsonb, jsonb, text, text, text, uuid, text, uuid, text) to authenticated;